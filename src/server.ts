...
function saveCredentials(apiKey: string, email: string) {
  mkdirSync(CREDENTIALS_DIR, { recursive: true });
  writeFileSync(
    CREDENTIALS_PATH,
    JSON.stringify({ api_key: apiKey, email }, null, 2)
  );
...
function getApiKey(): string {
  ...
  throw new Error(
    "Not authenticated. Use the revibe_login tool to log in, or set REVIBE_API_KEY in your MCP config."
  );
}
...
function detectGitHubUrl(): string | null {
  try {
  ...
  async () => {
    ...
    for (let i = 0; i < 60; i++) {
      ...
      const pollResp = await fetch(
        `${API_BASE}/cli-auth/poll?code=${device_code}`
      );
...
server.tool(
  ...
  "Submit a GitHub repository for Revibe analysis. Analyzes architecture, file roles, execution flows, system design Q&A, and more. If no URL is provided, auto-detects from the current git remote. Analysis takes 3-7 minutes for most repos.",
  {
    github_url: z
      ...
      .optional()
      .describe(
        "GitHub repository URL (e.g. https://github.com/owner/repo). Auto-detected if empty."
      ),
    reanalyze: z
      .boolean()
      .optional()
...
- flow_implementation — detailed execution flow code traces`,
  {
    ...
    section: z
      .string()
      .describe(
        'Section name (e.g. "architecture", "file_roles", "system_design_qa").'
      ),
  },
